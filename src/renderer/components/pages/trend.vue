...
export default {
  ...
  data() {
    return {
      fordata: [],
      tableTitle: [{title: '人员',key: '人员'},{title: '时间',key: '时间'},
    {title: '工作类别',key: '工作类别'},
    {title: '工作任务',key: '工作任务'},
    {title: '任务编号',key: '任务编号'},
  ...
  mounted(){console.log(this.tableData,"数据检查2",this.$store.state.standlib.data)
    ...
    get("/pdata/search/",{"m":members,"month":month}).then(res => {console.log(res)
    ...
    exportData(type) {
      if (type === 1) {
        this.$refs.table.exportCsv({
          filename: this.$store.state.setting.formDynamic.team+this.$store.state.setting.formDynamic.date+"量化绩效数据"
        });
      } else if (type === 2) {
        this.$refs.table.exportCsv({
